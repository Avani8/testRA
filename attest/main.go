...
func getClient(clientID int) (Client, error) {
	...
	var id int
                        ...
                        var sysvendor string
                        var prodname string
                        var prodversion string
                        var prodserial string
                        var produuid string    
                                    

	err := db.QueryRow(`SELECT * FROM client where id = $1`, clientID).Scan(&id, &hostname, &ip, &dt, &tm,&mac, &interfacename, &biosvendor, &biosdate, &biosversion, &sysvendor, &prodname, &prodversion,  &prodserial, &produuid)
	if err == nil {
		res = Client{ID: id, Hostname: hostname, IP: ip, Dt: dt, Tm: tm, MAC: mac, Interfacename: interfacename, Biosvendor: biosvendor, Biosdate: biosdate, Biosversion: biosversion, Sysvendor:sysvendor, Prodname: prodname, Prodversion:prodversion,  Prodserial:prodserial, Produuid:produuid}
	}
...
func allClients() ([]Client, error) {
	...
	if err == nil {
		for rows.Next() {
			...
			err = rows.Scan(&id, &hostname, &ip, &dt, &tm, &mac, &interfacename, &biosvendor, &biosdate, &biosversion, &sysvendor, &prodname, &prodversion,  &prodserial, &produuid)
		
			if err == nil {
				currentClient := Client{ID: id, Hostname: hostname, IP: ip, Dt: dt, Tm: tm,  MAC: mac, Interfacename: interfacename, Biosvendor: biosvendor, Biosdate: biosdate, Biosversion: biosversion, Sysvendor:sysvendor, Prodname: prodname, Prodversion:prodversion,  Prodserial:prodserial, Produuid:produuid}
...
func againresq(w http.ResponseWriter, r *http.Request) {
	...
	buf, err := ioutil.ReadFile("www/policystatus.html")
	if err != nil {
		renderErrorPage(w, err)
		//return certs,err
		return
...
func init() {
	tmpDB, err := sql.Open("postgres", "user=postgres password=myPassword dbname=aca sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	db = tmpDB
...
func main() {
	...
	http.HandleFunc("/policystatus.html",againresq)
